...
#         return self.table.infer_out_schema(schema)
    ...
    def rename_to_index(self, schema):
        pass
    ...
    def to_nameless(self, schema):
        return self.table.to_nameless(schema)
...
class TableReference(TableOp):
    ...
    def rename_to_index(self, schema):
#         mapping_dict = {}
#         for index, col in enumerate(schema[self.name]):
#             mapping_dict[col] = index
        tbl_schema = schema[self.name]
        tbl_name = self.alias if self.alias else self.name
        old_names = [set([c, f"{tbl_name}.{c}"]) for c in tbl_schema]
        mapping_dict = gen_name_mapping(old_names, range(len(tbl_schema)))
...
class JoinTable(TableOp):
    ...
    def rename_to_index(self, schema):
        ...
        self.left_tbl, mappings_left = self.left_tbl.rename_to_index(schema)
        self.right_tbl, mappings_right = self.right_tbl.rename_to_index(schema)
        ...
        self.rename_ops_nameless(mappings)
...
#         pprint(mappings)
    ...
    def rename_ops_nameless(self, mapping):
        if self.constraint:
            self.constraint.rename_ops_nameless(mapping)
...
#         if "left" in self.join_op:
#             op = "LEFT OUTER JOIN"
#             return f"({self.left_tbl.to_nameless(schema)} {op} {self.right_tbl.to_nameless(schema)} ON {self.constraint.to_nameless(schema) if self.constraint else ''})"
#         else:
#             op = "JOIN"
...
class SelectStatement(TableOp):
    ...
    def rename_to_index(self, schema):
        ...
        self.subquery_tree, child_mappings = self.subquery_tree.rename_to_index(schema)
        child_new_names = self.subquery_tree.infer_out_schema(schema)
        
#         print("My child mappings are", child_mappings)
        self.rename_ops_nameless(child_mappings, child_new_names)
...
#         pprint(mappings)
        ...
        self.rename_having_nameless(mappings_copy)
    ...
    def rename_ops_nameless(self, mapping, new_names):
        if type(self.columns[0]) == AllColumn:
            pass
        else:
            for col in self.columns:
                col.rename_ops_nameless(mapping)

        if self.where_tree:
            self.where_tree.rename_ops_nameless(mapping)
    ...
    def rename_having_nameless(self, mapping):
        if self.having_tree:
            self.having_tree.rename_ops_nameless(mapping)
    ...
    def to_nameless(self, schema):
#         where_part = "\nWHERE " + self.where_tree.to_nameless(schema) if self.where_tree else ""
#         group_part = "\nGROUP BY " + ", ".join([col.to_nameless(schema) + " " for col in self.group_col])+ ")" if self.group_col else "" # TODO: reference full names
#         if self.having_tree:
#             having_part = "\nHAVING " + self.having_tree.to_nameless(schema) # TODO reference full names
#         else:
#             having_part = ""
#         if type(self.columns[0]) == AllColumn:
#             select_part = "SELECT " + self.columns[0].to_nameless(schema, self.subquery_tree) + " "
#         else:
#             select_part = "SELECT " + ", ".join([col.to_nameless(schema) for col in self.columns]) + " "
#         return select_part + "\nFROM (" + self.subquery_tree.to_nameless(schema) + ")\n" + where_part + " " + group_part + " " + having_part 
        if type(self.columns[0]) == AllColumn:
            select_part = self.columns[0].to_nameless(schema, self.subquery_tree)
        else:
...
class Column(ColOp):
    ...
    def rename_ops_nameless(self, mapping):
        if self.table:
            self.index = mapping[self.table + "." + self.name] 
        else:
            self.index = mapping[self.name] 
    ...
    def rename_ops_nameless(self, mapping):
        for child in self.col_ops:
            if type(child) == AllColumn:
                key = next(iter(mapping))
                self.col_ops = [Column(None, False, direct = True, name = mapping[key])] # this is a hacky solution to replace count(*) with any column -- will not support NaNs!
                self.col_ops[0].index = 0
                break
            else:
                child.rename_ops_nameless(mapping)
...
class BinaryColumnOp(ColOp):
    ...
    def rename_ops_nameless(self, mapping):
        self.left_col_op.rename_ops_nameless(mapping)
        self.right_col_op.rename_ops_nameless(mapping)
...
class AllColumn(ColOp):
    ...
    def to_nameless(self, schema, table):
        table_schema = table.infer_out_schema(schema)
#         to_return = ""
...
class JoinPredicate(PredicateOp):
    ...
    def rename_ops_nameless(self, mapping):
        self.left_pred_op.rename_ops_nameless(mapping)
        self.right_pred_op.rename_ops_nameless(mapping)
...
def NotPred(PredicateOp):
    ...
    def rename_ops_nameless(self, mapping):
        self.pred_op.rename_ops_nameless(mapping)
